<template>
  <div class="cl-checklist">
    <div class="checklist" :class="{'show': isOpen}">
      ...
      <div class="desc">您已选择{{checkboxValue.length}}个，最多可选{{max}}个</div>
      <div class="list" ref="list">
        <div v-for="(item, index) in dataList" class="line-wrapper" :data-val="item.label + '|' + item.value" :key="index">
          ...
          <input type="checkbox" :id="index" v-model.trim="checkboxValue" @click="selectedItem($event)" style="display: none" :value="item.label + '|' + item.value">
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    hide() {
      this.isOpen = false;
      this.checkboxValue = Array.from(this.oldCheckboxValue);
    },
    onConfirm() {
      this.isOpen = false;
  ...
  mounted() {
    this.oldCheckboxValue = Array.from(this.checkboxValue);
  }
};
</script>
<style scoped>
